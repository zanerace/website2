...
/** Production Formspree form; override with VITE_FORMSPREE_ID for staging if needed */
const FORMSPREE_FORM_ID =
  import.meta.env.VITE_FORMSPREE_ID?.trim() || 'mbdweard';
...
function AuditContactForm() {
  ...
  if (state.succeeded) {
    return (
      <div className="glass-surface-strong rounded-2xl p-8 md:p-10 min-h-[min(72dvh,520px)] sm:min-h-[420px] flex flex-col items-center justify-center text-center">
        ...
        <p className="font-grotesk font-light text-white/40 text-base max-w-sm mb-8">
          I'll review your online presence and get back to you within 24 hours
          with actionable feedback.
        </p>
        <button
          type="button"
          onClick={() => reset()}
          className="font-grotesk font-medium text-white/80 border border-white/20 hover:border-gold/40 hover:text-gold text-sm tracking-wide px-6 py-3 rounded-full transition-colors"
        >
  ...
  return (
    ...
    >
      ...
      <div className="space-y-4 flex-1">
        ...
        <div>
          ...
          <ValidationError
            errors={state.errors}
            field="name"
            className="font-grotesk text-red-400/90 text-sm mt-2"
          />
        </div>
        ...
        <div>
          <label className="font-grotesk font-medium text-white/50 text-[11px] tracking-[0.2em] uppercase mb-2 block">
            Website / Social / Google Link <span className="text-gold/60">*</span>
          </label>
          <input
            type="text"
            name="link"
      ...
      <ValidationError
        errors={state.errors}
        className="font-grotesk text-red-400/90 text-sm mt-4"
      />
      ...
      <p className="font-grotesk text-white/25 text-xs mt-4 text-center">
